...
/*
TweetUnlikeParams contains all the parameters to send to the API endpoint
for the get user by name operation typically these are written to a http.Request
*/
type TweetUnlikeParams struct {
	TweetId int32
}

// WriteToRequest writes these params to a swagger request
// API: POST /api/social/tweet/{tweet_id}/unlike
func (o *TweetUnlikeParams) WriteToRequest(r client.Request, reg strfmt.Registry) error {
	// path param name
	if err := r.SetPathParam("social", "tweet"); err != nil {
		return err
	}
	if err := r.SetPathParam(
			fmt.Sprintf("%d", o.TweetId), "unlike"); err != nil {
		return err
	}
